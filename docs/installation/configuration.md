...
Despite the task having a [common structure](../articles/state-machine.md), there are small variations to the task that can be achieved by tweaking inumerous configurations. Additionally, there's a need to input the calibration parameters for different pieces of hardware (speakers, SyringePumps, etc), which vary from setup to setup. Because of this, 3 different configuration files were created:
...
- `[animal]_YYMMDD_hhmmss.mp4` - this is the video recorded during the session.
- `cam_metadata_hhmmss.csv` - this is the file containing metadata directly saved from the camera for every frame recorded in `[animal]_YYMMDD_hhmmss.mp4`, so for each frame it's possible to know the camera timestamp, the frame ID and the state of the camera GPIO pins.
